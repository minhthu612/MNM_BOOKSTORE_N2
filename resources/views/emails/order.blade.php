...
<body style="font-family: sans-serif; line-height: 1.6; color: #333; margin: 0; padding: 20px;">
    ...
    <p>Chào <strong>{{ $user->fullname ?? Auth::user()->fullname }}</strong>,</p>
        ...
        <tfoot>
            <tr>
                <td colspan="2" style="padding: 10px; border: 1px solid #ddd; text-align: right;">
                    <strong>{{ $order->status == 'cancelled' ? 'Số tiền hủy:' : 'Tổng cộng:' }}</strong>
                </td>
    ...
    @if($order->status == 'cancelled')
        ...
        <p style="margin: 15px 0;">Nếu có thắc mắc, vui lòng liên hệ hotline của chúng tôi ngay lập tức.</p>
    @else
